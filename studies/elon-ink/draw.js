(() => {
  ...
  const WIDTH = 420, HEIGHT = 510, RESOLUTION = 4;
  ...
  // A seeded distribution of tiny black marks models the ink's density.
  function stipple(shape, bounds, fields, count, base = 0.01, radius = 0.43) {
    ctx.save();
    ctx.clip(typeof shape === 'string' ? path(shape) : shape);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const r = radius * between(0.35, 1.25);
      ctx.globalAlpha = between(0.40, 0.85);
      ctx.beginPath();
      ctx.ellipse(x, y, r, r * between(0.65, 1.25), random() * Math.PI, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  fill(leftEar, 1, '#fff');
  fill(rightEar, 1, '#fff');
  stipple(rightEar, [738, 214, 44, 101], [[758, 258, 15, 35, 0.35], [747, 282, 10, 20, 0.35]], 6500, 0.05, 0.4);
  ...
  stipple(face, [511, 83, 247, 357], [
    ...
    [568, 220, 28, 15, 0.21], [567, 247, 25, 9, 0.22],
    [667, 216, 24, 11, 0.12], [669, 245, 30, 10, 0.12],
  ...
  const browLeft = 'M541 209 C551 201 560 200 571 204 C580 207 589 211 595 218 L591 220 C574 212 562 207 545 212 Z';
  const browRight = 'M642 211 C655 203 668 197 680 199 C690 201 699 205 705 210 L698 212 C686 205 675 204 666 207 L645 215 Z';
  fill(browLeft, 0.24);
  fill(browRight, 0.24);
  for (let i = 0; i < 78; i++) {
    const t = i / 77;
    let x = 543 + t * 50;
    let y = 209 - 6 * Math.sin(t * Math.PI) + 9 * t;
    line(`M${x} ${y + between(-0.7, 1.5)} q${between(-2, 2)} ${between(-2, -5)} ${between(1, 4)} ${between(-4, -7)}`, between(0.3, 0.63), between(0.3, 0.65));
    x = 643 + t * 58;
    y = 214 - 11 * Math.sin(t * Math.PI) - 2 * t;
  ...
  function eye(outline, x, y, radius, top, lower, crease) {
    ...
    ctx.save();
    ctx.clip(mask);
    ellipse(x, y, radius, radius, '#909090');
    for (let k = 0; k < 34; k++) {
      const a = k / 34 * Math.PI * 2;
      const r = radius * 0.58;
      line(`M${x + Math.cos(a) * r} ${y + Math.sin(a) * r} L${x + Math.cos(a) * radius} ${y + Math.sin(a) * radius}`, 0.32, 0.45);
    }
  ...
  eye(
    'M549 236 C558 229 568 228 579 231 C586 232 592 234 595 237 C584 243 564 246 549 236 Z',
    572, 235, 7.6,
    'M549 236 C558 229 568 228 579 231 C586 232 592 234 595 237',
    'M551 237 C565 245 582 244 592 239',
    'M546 230 C558 218 581 219 593 230'
  );
  eye(
    'M642 234 C651 229 662 227 674 228 C682 229 688 231 692 234 C680 240 656 244 642 234 Z',
    666, 232.7, 7.4,
    'M642 234 C651 229 662 227 674 228 C682 229 688 231 692 234',
    'M646 238 C659 244 681 241 689 237',
    'M641 227 C653 217 676 216 689 226'
  );
  ...
  const upperLip = 'M577 341 C591 337 603 332 613 333 C620 332 625 336 630 334 C645 332 664 334 681 339 C669 343 651 347 632 349 C610 350 592 347 577 344 Z';
  fill(upperLip, 0.12);
  hatch(upperLip, 1.35, 0.75, 0.43, 0.42);
  const opening = 'M578 342 C595 341 610 339 627 340 C645 341 664 340 681 339 C668 347 650 353 630 352 C609 352 592 348 578 345 Z';
  fill(opening, 0.90);
  fill('M595 343 C611 341 629 341 645 342 L643 347 C630 350 609 349 596 347 Z', 1, '#fff');
  line('M607 342 L607 347 M620 342 L620 348 M634 342 L634 347', 0.33, 0.42);
  const lowerLip = 'M587 347 C600 350 614 353 631 353 C650 353 665 347 677 342 C668 354 654 363 638 365 C617 368 599 360 587 347 Z';
  fill(lowerLip, 0.035);
  stipple(lowerLip, [585, 342, 99, 28], [[626, 364, 30, 7, 0.46], [599, 352, 12, 9, 0.16]], 5200, 0.01, 0.28);
  line('M592 352 C601 362 617 367 632 366 C648 365 661 358 668 352', 0.64, 0.43);
  line('M606 355 C620 360 641 360 653 355', 0.52, 0.90, '#fff');
  line('M577 342 C575 343 575 346 578 349 M681 339 L685 342', 0.95, 0.65);
